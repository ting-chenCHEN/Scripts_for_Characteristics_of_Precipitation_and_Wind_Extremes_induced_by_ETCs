...
! ===========================================
  ...
  do n = 1, nummax
     ...
     do j = 1, jx
        ...
        if (Wex_mask(i,j,n).gt.0.5) Wex_avg(n)=Wex_avg(n)+Wextreme_weighted(i,j,n)/NNA_area
        if (Pex_mask(i,j,n).gt.0.5) Pex_avg(n)=Pex_avg(n)+Pextreme_weighted(i,j,n)/NNA_area
...
open (20, FILE='/pampa/chen/local_extremes/&
        ...
        _TPp'//TPpercentile_str//'_timecum.txt',&
        STATUS='replace')
