...
! Read in the tracked storm data
  ...
  do line = 1, linemax
    ...
    if (line .le. lineheader) then
        ...
        read(10,100)  num, point, yyyy_int, mm_int, dd_int, hh_int, &
                      ...
                      !latc(num,point), lonc(num,point) !, vorm(num,point)
        print*,'num=',num
        !if (yyyy_int.eq.2000) cycle 
        if (num.lt.num_pre0) cycle 

        !print*,'reading storm file line=',line,'yyyy_int=',yyyy_int,'mm_int=', mm_int   
        ...
        ELSE
           ...
           if (yyyy_int.ne.ys.and.mm_int.ne.ms) then
              !print*,'close the previous files'
  ...
  do t = 1, timeranges
     ...
     print*,'t=',t,'Tpdupdf_avg=', TPdupdf_avg(t),'cont=',TPdupdf_avg_C(t)
     do s = 1, 4
...
!fileout0='TPExtremes_p'//TPpercentile_str//'_duration_exceedance_distribution_allETCs.nc'
!call writegrid(fileout0,glons0,glats0, TPduration_avg, TPduration_season, TPexceedance_avg, TPexceedance_season, ix, jx, 4)

fileout1='PDF_TPexceedance_duration_p'//TPpercentile_str//'_NNA_ERA5.txt'
