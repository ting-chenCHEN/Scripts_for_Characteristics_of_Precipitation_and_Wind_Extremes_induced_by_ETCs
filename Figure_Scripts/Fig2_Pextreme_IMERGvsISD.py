...
# IMERG (regridded to ISD)
reference_date = dt.datetime(2001, 1, 1, 0, 0, 0)  # YYYY, MM, DD, HH, MM, SS
filet='IMERG_regrissed_ISDstation_TP_p99p0.nc4'
da=xr.open_dataset(fiet)
...
for s in seasons: 
    ...
    cf = plt.scatter(lons, lats, c=zzera5, s=20, cmap=cmap,norm=norm, edgecolor='gray',linewidth=0.5,
            transform=ccrs.PlateCarree(),zorder=11,alpha=0.95)
    ...
    if s==0:
       ax.text(0.5,1.03,'IMERG',horizontalalignment='center',transform=ax.transAxes,fontsize=13,zorder=24,weight='bold',**mono_font)
...
plt.savefig('Fig2_Precip_ex990values_freq_ISD_vs_IMERG', bbox_inches='tight', dpi=200)
plt.show()
plt.close()
da.close()
ds.close()
